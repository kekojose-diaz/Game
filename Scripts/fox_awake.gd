...
@onready var ray_cast_awake_left: RayCast2D = $RayCast_awake_left
@onready var ray_cast_awake_right: RayCast2D = $RayCast_awake_right

# Called every frame. 'delta' is the elapsed time since the previous frame.
func _process(delta):
	if sleeping:
		if ray_cast_awake_left.is_colliding():
			sleeping = false
			direction = -1
			animated_sprite.play("Walking")
			animated_sprite.flip_h = true
		elif ray_cast_awake_right.is_colliding():
			sleeping = false
			direction = 1
			animated_sprite.play("Walking")
